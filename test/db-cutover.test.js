...
test('LEGACY_SNAPSHOT_WRITE=false: legacy snapshot row count is unchanged by a real crawl (#20.Q)', () => {
  ...
  const script = `
    ...
    const Database = require('better-sqlite3');
    const raw = new Database('${path.join(PROJECT_ROOT, 'data', 'collector.db').replace(/\\/g, '\\\\')}');
    raw.pragma('busy_timeout = 5000');
    const countSnapshots = () => raw.prepare("SELECT COUNT(*) c FROM snapshots WHERE platform = ? AND query = ?").get(platform, query).c;

    const legacyBefore = countSnapshots();
    const run = db.createRun({ platform, query, maxItems: 2 });
    db.insertSnapshots(run.id, platform, query, [
      { title: 'A', url: 'https://example.com/cutover-q-${suffix}/a', likes: 1 },
      { title: 'B', url: 'https://example.com/cutover-q-${suffix}/b', likes: 2 },
    ]);
    const legacyAfter = countSnapshots();
    db.deleteRun(run.id);
    raw.close();
...
test('With legacy writes OFF, new/active/dropped stay correct across two sequential Runs (#12/#20.R)', () => {
  ...
  const script = `
    ...
    const run1 = db.createRun({ platform, query, maxItems: 2, requestedBackend: null });
    db.updateRun(run1.id, { status: 'done' }); // must be 'done' so the NEXT run can find it as the comparison baseline
    const result1 = db.insertSnapshots(run1.id, platform, query, [
    ...
    const run2 = db.createRun({ platform, query, maxItems: 2 });
    const result2 = db.insertSnapshots(run2.id, platform, query, [
...
test('getProductCurrentByUid exposes real non-zero growth across two sequential crawls, the source export now reads under READ_MODEL_V2 (#11)', () => {
  const suffix = Date.now();
  const platform = 'test';
  ...
  const run1 = db.createRun({ platform, query, maxItems: 1 });
  db.updateRun(run1.id, { status: 'done' });
  db.insertSnapshots(run1.id, platform, query, [
    { title: 'Growth Item', url, price: 10, views: 100, likes: 5, soldCount: 3, reviews: 20 }
  ]);

  const afterFirstCrawl = db.getProductCurrentByUid(itemUid);
  ...
  const run2 = db.createRun({ platform, query, maxItems: 1 });
  db.insertSnapshots(run2.id, platform, query, [
    { title: 'Growth Item', url, price: 12, views: 100, likes: 9, soldCount: 5, reviews: 25 }
  ]);

  const afterSecondCrawl = db.getProductCurrentByUid(itemUid);
  try {
    ...
    raw.pragma('busy_timeout = 5000');
    raw.prepare('DELETE FROM product_current WHERE item_uid = ?').run(itemUid);
    raw.prepare('DELETE FROM daily_packed_history WHERE item_uid = ?').run(itemUid);
    raw.prepare('DELETE FROM snapshots WHERE item_uid = ?').run(itemUid);
    raw.close();
    db.deleteRun(run1.id);
    db.deleteRun(run2.id);
  }
...
test('checkV2Parity detects an exact metric mismatch inside a correctly-identified packed observation, including reviews (Gap #3)', () => {
  const suffix = Date.now();
  const platform = 'test';
  const query = 'gap3-metric-' + suffix;
  const url = `https://example.com/gap3-metric-${suffix}/a`;
  const itemUid = `${platform}:${url}`;
  const run = db.createRun({ platform, query, maxItems: 1 });
  const raw = new Database(DB_PATH);
  raw.pragma('busy_timeout = 5000');
  try {
    db.insertSnapshots(run.id, platform, query, [
    ...
    db.backfillSnapshotsToV2();

    const legacySnap = raw.prepare('SELECT * FROM snapshots WHERE item_uid = ? ORDER BY id DESC LIMIT 1').get(itemUid);
    assert.ok(legacySnap, 'legacy snapshot must exist');
    const expectedObservationId = `legacy:${legacySnap.id}`;

    const packedRow = raw.prepare('SELECT * FROM daily_packed_history WHERE item_uid = ?').get(itemUid);
    ...
    raw.prepare('UPDATE daily_packed_history SET observations_json = ? WHERE item_uid = ?').run(JSON.stringify(observations), itemUid);

    const parity = db.checkV2Parity();
...
test('checkV2Parity detects a full-metric mismatch beyond price/likes (#13/#20.O)', () => {
  const suffix = Date.now();
  const platform = 'test';
  ...
  try {
    ...
    raw.prepare('UPDATE product_current SET current_views = ? WHERE item_uid = ?').run(9999, itemUid);
...
test('checkV2Parity detects missing historical observations (packed count < legacy count) (#13/#20.P)', () => {
  const suffix = Date.now();
  const platform = 'test';
...
test('checkV2Parity: a duplicate observation makes parityOk false, not just visible in the count (#5)', () => {
  const suffix = Date.now();
  const platform = 'test';
  ...
  raw.pragma('busy_timeout = 5000');
  try {
    db.insertSnapshots(run.id, platform, query, [{ title: 'Dup Item', url, price: 1, views: 1, likes: 1 }]);
    const row = raw.prepare('SELECT observations_json FROM daily_packed_history WHERE item_uid = ?').get(itemUid);
    const obs = JSON.parse(row.observations_json);
    const duped = [...obs, { ...obs[0] }]; // force a real duplicate: same observationId AND same time
    raw.prepare('UPDATE daily_packed_history SET observations_json = ?, observation_count = ? WHERE item_uid = ?')
...
test('checkV2Parity detects timestamp mismatch between legacy snapshot and V2 observation (Gap #5)', () => {
  const suffix = Date.now();
  const platform = 'test';
  ...
  try {
    ...
    raw.prepare('UPDATE daily_packed_history SET observations_json = ? WHERE item_uid = ?')
...
test('checkV2Parity detects global duplicate observationId appearing across different daily rows (Gap #5)', () => {
  const suffix = Date.now();
  const platform = 'test';
...
test('checkV2Parity flags malformed when observation is missing a required schema key (Gap #5)', () => {
  const suffix = Date.now();
  const platform = 'test';
...
test('checkV2Parity fails parity on asymmetric null metrics between legacy and V2 (Patch #2A)', () => {
  const suffix = Date.now();
  const platform = 'test';
  ...
  try {
    ...
    const parity1 = db.checkV2Parity();
    ...
    raw.prepare('UPDATE snapshots SET reviews = NULL WHERE id = ?').run(legacySnap.id);
    targetObs.reviews = 20;
    raw.prepare('UPDATE daily_packed_history SET observations_json = ? WHERE item_uid = ?')
      .run(JSON.stringify(obs), itemUid);

    const parity2 = db.checkV2Parity();
    ...
    const parity3 = db.checkV2Parity();
...
test('Backfill with SQLite UTC timestamp string produces exact UTC observation and 0 timestamp mismatch (Patch #2B)', () => {
  const suffix = Date.now();
  const platform = 'test';
  ...
  try {
    ...
    const res = db.backfillSnapshotsToV2();
    assert.ok(res.migrated >= 1);

    const row = raw.prepare('SELECT date, observations_json FROM daily_packed_history WHERE item_uid = ?').get(itemUid);
    assert.ok(row);
