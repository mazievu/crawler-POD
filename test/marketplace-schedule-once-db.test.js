...
test('a one-time schedule stores its Vietnam date and disables itself after completion', () => {
  ...
  try {
    ...
    assert.equal(db.completeMarketplaceCaptureSchedule(schedule.id, { captured: 30 }, new Date('2099-12-31T16:46:00.000Z')), true);
    const completed = db.getMarketplaceCaptureSchedules().find((candidate) => candidate.id === schedule.id);
    assert.equal(completed.enabled, 0);
    assert.deepEqual(completed.last_summary, { captured: 30 });
    const runs = db.getMarketplaceCaptureScheduleRuns(schedule.id);
