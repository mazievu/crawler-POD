...
test('Daily Packed History appendObservation is idempotent by observationId (#4.2/#16.H)', () => {
  ...
  const r1 = ops.appendObservation(item, ts, { runId: 100 });
  assert.equal(r1.duplicate, false);
  const r2 = ops.appendObservation(item, ts, { runId: 100 }); // identical identity — must replace, not duplicate
  assert.equal(r2.duplicate, true);
  const r3 = ops.appendObservation(item, ts, { runId: 100 }); // run it a third time for good measure
  assert.equal(r3.duplicate, true);

  const row = ops.findRow('test:idempotent-item', '2026-08-25');
  ...
  ops.appendObservation(item, ts, { runId: 101 });
  const rowAfter = ops.findRow('test:idempotent-item', '2026-08-25');
...
test('Daily Packed History appendObservation is idempotent for legacy migration identity (#4.1)', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  const ops = createDailyHistoryOps(db);
  ...
  ops.appendObservation(item, ts, { legacySnapshotId: 555 });
  ops.appendObservation(item, ts, { legacySnapshotId: 555 }); // re-running the migration for the same legacy row

  const row = ops.findRow('test:legacy-migrated-item', '2026-08-25');
...
test('Product Current guarantees 1 row per item_uid and tracks deltas', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  ...
  const r1 = ops.upsertItem(item1, 1);
  assert.equal(r1.isNew, true);

  const row1 = ops.findByUid('shopify:sku_123');
  ...
  const r2 = ops.upsertItem(item2, 2);
  assert.equal(r2.isNew, false);

  const row2 = ops.findByUid('shopify:sku_123');
  ...
  const count = db.prepare('SELECT COUNT(*) as cnt FROM product_current').get();
...
test('Product Current computes real windowed 3h/24h deltas from Daily History, not previous-crawl deltas (P0-4)', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  const dailyOps = createDailyHistoryOps(db);
  const ops = createProductCurrentOps(db, dailyOps);
  ...
  ops.upsertItem({ item_uid: uid, platform: 'reddit', views: 50, likes: 5 }, 1, new Date('2026-08-23T15:00:00Z'));
  dailyOps.appendObservation({ item_uid: uid, platform: 'reddit', views: 50, likes: 5 }, new Date('2026-08-23T15:00:00Z'));

  // Today 09:00, 12:00 observations.
  ops.upsertItem({ item_uid: uid, platform: 'reddit', views: 100, likes: 10 }, 2, new Date('2026-08-24T09:00:00Z'));
  dailyOps.appendObservation({ item_uid: uid, platform: 'reddit', views: 100, likes: 10 }, new Date('2026-08-24T09:00:00Z'));

  ops.upsertItem({ item_uid: uid, platform: 'reddit', views: 150, likes: 15 }, 3, new Date('2026-08-24T12:00:00Z'));
  dailyOps.appendObservation({ item_uid: uid, platform: 'reddit', views: 150, likes: 15 }, new Date('2026-08-24T12:00:00Z'));
  ...
  const result = ops.upsertItem({ item_uid: uid, platform: 'reddit', views: 220, likes: 22 }, 4, new Date('2026-08-24T15:00:00Z'));

  assert.equal(result.deltas.delta_24h_views, 170, '220 - 50 (yesterday 15:00)');
  const row = ops.findByUid(uid);
...
test('Product Current returns null (not 0) for windowed deltas when no observation exists in tolerance window (P0-4)', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  const dailyOps = createDailyHistoryOps(db);
  const ops = createProductCurrentOps(db, dailyOps);
  ...
  ops.upsertItem({ item_uid: uid, platform: 'twitter', views: 10, likes: 1 }, 1, new Date('2026-08-24T09:00:00Z'));
  dailyOps.appendObservation({ item_uid: uid, platform: 'twitter', views: 10, likes: 1 }, new Date('2026-08-24T09:00:00Z'));

  // Next crawl only 20 minutes later - no observation exists near the 3h or 24h mark.
  const result = ops.upsertItem({ item_uid: uid, platform: 'twitter', views: 12, likes: 2 }, 2, new Date('2026-08-24T09:20:00Z'));
  assert.equal(result.deltas.delta_3h_views, null);
  assert.equal(result.deltas.delta_24h_views, null);
});

test('Daily Packed History aggregates intra-day observations into 1 daily row', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  const dailyOps = createDailyHistoryOps(db);
  ...
  dailyOps.appendObservation({ ...baseItem, likes: 10, views: 100 }, d1);
  dailyOps.appendObservation({ ...baseItem, likes: 25, views: 250 }, d2);
  dailyOps.appendObservation({ ...baseItem, likes: 60, views: 600, price: 29.0 }, d3);

  const history = dailyOps.getHistory('etsy:item_999');
...
test('Weekly Summary uses sum/count average and last-minus-first deltas, not running-average math (P0-5)', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  ...
  weeklyOps.updateWeekly({ item_uid: uid, platform: 'tiktok', price: 10, likes: 100, views: 1000, sold_count: 5 }, monday);
  weeklyOps.updateWeekly({ item_uid: uid, platform: 'tiktok', price: 20, likes: 150, views: 1500, sold_count: 8 }, monday);
  weeklyOps.updateWeekly({ item_uid: uid, platform: 'tiktok', price: 30, likes: 300, views: 3000, sold_count: 20 }, monday);

  const rollups = weeklyOps.getWeekly(uid);
...
test('Weekly Summary growth_rate avoids divide-by-zero when first_views is 0', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  const weeklyOps = createWeeklySummaryOps(db);
  ...
  weeklyOps.updateWeekly({ item_uid: uid, platform: 'facebook', price: 5, likes: 0, views: 0, sold_count: 0 }, t);
  weeklyOps.updateWeekly({ item_uid: uid, platform: 'facebook', price: 5, likes: 10, views: 50, sold_count: 1 }, t);

  const week = weeklyOps.getWeekly(uid)[0];
  assert.equal(week.growth_rate, 0, 'No baseline (first_views=0) -> defined as 0, not Infinity/NaN');
});

test('recomputeWeeklySummaryFromHistory rebuilds weekly_summary from Daily History without touching it', () => {
  const db = new Database(':memory:');
  initSchemaV2(db);
  const dailyOps = createDailyHistoryOps(db);
  const uid = 'shopify:rebuild_1';

  dailyOps.appendObservation({ item_uid: uid, platform: 'shopify', price: 10, likes: 5, views: 100, sold_count: 1 }, new Date('2026-08-24T08:00:00Z'));
  dailyOps.appendObservation({ item_uid: uid, platform: 'shopify', price: 12, likes: 20, views: 400, sold_count: 4 }, new Date('2026-08-24T20:00:00Z'));

  const before = dailyOps.getHistory(uid);
  assert.equal(before[0].observations.length, 2);

  const result = recomputeWeeklySummaryFromHistory(db);
  assert.ok(result.weeksRebuilt >= 1);

  const after = dailyOps.getHistory(uid);
